<!-- atlas:onramp v0.1 -->
### Working with the Atlas (`llm-armory-mind/`)

`llm-armory-mind/` is this repository's knowledge base — an Obsidian-compatible
vault that is the single source of *understanding*, kept separate from the
code it describes.

- **Orient Atlas-first.** Before working in an area, read
  `llm-armory-mind/map/index.md`, then the relevant
...
- **Pipeline.** Route spec-writing output to `llm-armory-mind/specs/` and
  plan-writing output to `llm-armory-mind/plans/` (memory-atlas vault —
